...
def WriteStreamNetwork(network,para):
    ...
    if (network == "tcp" and para=="none"):
        streamfile=file("/usr/local/V2Proxy/json_template/tcp.json")
        tcp=json.load(streamfile)
        config[u"inbound"][u"streamSettings"]=tcp

    if (network == "tcp" and para != "none"):
        streamfile=file("/usr/local/V2Proxy/json_template/http.json")
    ...
    if (network == "ws"):
        streamfile=file("/usr/local/V2Proxy/json_template/ws.json")
...
def WriteTLS(action,domain):
    if action == "on":
        ...
        tls_file = file("/usr/local/V2Proxy/json_template/tlssettings.json")
        ...
        domainfile = file("/usr/local/V2Proxy/mydomain", "w+")
